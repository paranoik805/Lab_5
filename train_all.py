...
def process_data(image, label):
   return tf.image.per_image_standardization(image),labe

def contrast (image, label):
   return tf.image.adjust_contrast(image, 2), label

def brightness(image, label):
   return tf.image.adjust_brightness(image, delta = 0.25), label
...
def unfreeze_model(model):
	for layer in model.layer[-20]:
		if not isinstance(layers,tf.keras.layers.BatchNormalization):
